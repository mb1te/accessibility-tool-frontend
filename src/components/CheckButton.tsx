...
const CheckButton: FC<CheckButtonProps> = ({name, isActive, isCompleted, onClick}) => {
    const getColor = () => {
    ...
    const getClassNames = () => {
        const names = ["check-main__check-list"];
        if (isActive) names.push("check-list-active")
        if (isCompleted) names.push()
        return names.join(" ");
    }

    return (
      <button onClick={onClick} className={getClassNames()}>
        <svg className="check-main__check-list-svg" width="2vw" height="2vw" viewBox="0 -1 29 29" fill="none"
